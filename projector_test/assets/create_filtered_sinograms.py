...
def filter_img(sinogram):
    pad_size = sinogram.shape[1] // 2
    sino_pad = np.pad(sinogram, ((0, 0), (pad_size, pad_size)), mode="constant")  # for correct FFT operations

    detector_size = sino_pad.shape[1]

    fft_filter = np.fft.fftfreq(detector_size, 1. / detector_size)
    fft_filter = fft_filter / np.max(fft_filter)
    fft_filter = np.abs(fft_filter)

    sino_shift = np.fft.ifftshift(sino_pad, axes=1)
    sino_fft = np.fft.fft(sino_shift, axis=1)
    sino_filtered = sino_fft * fft_filter[None, :]
    sino_filtered = np.real(np.fft.ifftshift(np.fft.ifft(sino_filtered, axis=1), axes=1))
    sino_filtered = sino_filtered[:, pad_size + sinogram.shape[1] % 2:sinogram.shape[1] + pad_size + sinogram.shape[1] % 2]
...
def calculate_sino_2D(geometry, algo, model):
    ...
    sinogram_bmp_path = "filtered_sinograms/sinogram_2D_"+model+"_"+geometry+"_"+algo+".bmp"
    sinogram_txt_path = "filtered_sinograms/sinogram_2D_"+model+"_"+geometry+"_"+algo+".txt"
    ...
    sinogram = filter_img(sinogram)
    with open(sinogram_txt_path, "w") as f :
        f.write(str(detector_size_x) + " " + str(angles.shape[0]) + "\n")
        for i in range(len(angles)):
            f.write(" ".join([str(x) for x in sinogram[i, :].tolist()]))
            f.write("\n")
    sinogram = np.floor((sinogram-sinogram.min())/(sinogram.max()-sinogram.min())*255.0)
...
def calculate_sino_3D(model, geometry):
    ...
    sinogram_txt_path = "filtered_sinograms/sinogram_3D_"+model+"_"+geometry+"_line.txt"
    sinogram_bmp_path = "filtered_sinograms/sinogram_3D_"+model+"_"+geometry+"_line.bmp"
    ...
    with open(sinogram_txt_path, "w") as f:
        ...
        #f.write("\n")
    sino_arr = np.floor((sino_arr.T-sino_arr.min())/(sino_arr.max()-sino_arr.min())*255.0)
